...
export function unwrappedToken(token: Token): Currency {
  if (token.equals(WAVAX[token.chainId])) return CAVAX[token.chainId]
  return token
}

export function unwrappedTokenV3(token: Token, chainId: ChainId): Currency | Token {
  if (token?.equals?.(WAVAX[token.chainId])) return CAVAX[chainId];
  return token;
}

export function unwrappedTokenV3W(token: Token, chainId: ChainId): Token {
  return token;
}
