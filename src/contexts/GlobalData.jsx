...
import { getPercentChange, getBlocksFromTimestamps, get2DayPercentChange, getTimeframe } from '../utils/analytics'
...
async function getGlobalData(ethPrice, oldEthPrice) {
  ...
  try {
    ...
    data = result.data.canaryFactories[0]
    ...
    oneDayData = oneDayResult.data.canaryFactories[0]

    const twoDayResult = await client.query({
      query: GLOBAL_DATA(twoDayBlock?.number),
      fetchPolicy: 'cache-first'
    })
    twoDayData = twoDayResult.data.canaryFactories[0]

    const oneWeekResult = await client.query({
      query: GLOBAL_DATA(oneWeekBlock?.number),
      fetchPolicy: 'cache-first'
    })
    const oneWeekData = oneWeekResult.data.canaryFactories[0]

    const twoWeekResult = await client.query({
      query: GLOBAL_DATA(twoWeekBlock?.number),
      fetchPolicy: 'cache-first'
    })
    const twoWeekData = twoWeekResult.data.canaryFactories[0]

    if (data) {
      //if (data && oneDayData && twoDayData && twoWeekData) {

      // format the total liquidity in USD
      data.totalLiquidityUSD = data.totalLiquidityETH * ethPrice //+ 328000 //+staking
...
const getChartData = async (oldestDateToFetch, ethPrice) => {
  ...
  try {
    while (!allFound) {
      ...
      skip += 1000
      data = data.concat(result.data.canaryDayDatas)
      if (result.data.canaryDayDatas.length < 1000) {
        allFound = true
      }
    }
...
const getEthPrice = async () => {
  ...
  const ethPrice = 18 //result['data']['avalanche-2']['usd']
  const priceChangeETH = 0 //result['data']['avalanche-2']['usd_24h_change']
...
export const getEthPriceAtTimestamp = async timestamp => {
  ...
  const diff = utcCurrentTime - timestamp
  const days_back = Math.round(diff / (60 * 60 * 24))

  const result = await coinGeckoClient.coins.fetchMarketChart('avalanche-2', {
    days: days_back,
    vs_currency: 'usd'
  })
...
export function useGlobalChartData() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const [newChartData, newWeeklyData] = await getChartData(oldestDateFetch, ethPrice)
