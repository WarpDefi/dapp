    ...
    return (
      ...
      >
        <Aligner buttonStyle={buttonStyle} active={Boolean(currency && currency.symbol)}>
          {renderCurrency()}
          {renderStyletoken()}
          {showArrowIcon && <Icons.chevronDown color={!Boolean(currency && currency.symbol) ? 'black' : undefined} />}
        </Aligner>
      </CurrencySelect>
    );
  };

  return <div>{isShowTextInput ? <TextInput {...(rest as any)} addonAfter={addonAfter()} /> : addonAfter()}</div>;
};
