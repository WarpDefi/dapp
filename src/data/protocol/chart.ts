...
import { arbitrumClient, avalancheClient, optimismClient } from '@/apolloInfo/client'
...
async function fetchChartData(client: ApolloClient<NormalizedCacheObject>) {
  ...
  try {
    while (!allFound) {
      ...
      if (!loading) {
        skip += 1000
        if (chartResData.pangolinDayDatas.length < 1000 || error) {
          allFound = true
        }
        if (chartResData) {
          data = data.concat(chartResData.pangolinDayDatas)
        }
      }
    }
  } catch {
...
export function useFetchGlobalChartData(): {
  ...
  const shouldUserDerivedData =
    activeNetworkVersion.id === SupportedNetwork.AVALANCHE
