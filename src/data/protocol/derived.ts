...
import { useActiveNetworkVersion, useDataClient } from '@/state/applicationInfo/hooks'
    ...
    async function fetchAll() {
      ...
      const data = await POOL_HIDE[currentNetwork.id].reduce(
        async (accumP: Promise<{ [key: number]: ChartDayData }>, address) => {
          ...
          dispatch(updatePoolChartData({ poolAddress: address, chartData: data, networkId: SupportedNetwork.AVALANCHE }))
