...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      expect(txs[ChainId.AVALANCHE]).toBeTruthy()
      expect(txs[ChainId.AVALANCHE]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.AVALANCHE]?.['0x0']
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.AVALANCHE), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.AVALANCHE] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.AVALANCHE}))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.AVALANCHE), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.AVALANCHE] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
    })
  })
})
