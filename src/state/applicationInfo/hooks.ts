...
export function useBlockNumber(): number | undefined {
  ...
  return useSelector((state: AppState) => state.applicationInfo.blockNumber[chainId ?? -1])
}

export function useModalOpen(modal: ApplicationModal): boolean {
  const openModal = useSelector((state: AppState) => state.applicationInfo.openModal)
...
export function useActivePopups(): AppState['application']['popupList'] {
  const list = useSelector((state: AppState) => state.applicationInfo.popupList)
...
export function useSubgraphStatus(): [
  ...
  const dispatch = useDispatch()
  const status = useSelector((state: AppState) => state.applicationInfo.subgraphStatus)
...
export function useActiveNetworkVersion(): [NetworkInfo, (activeNetworkVersion: NetworkInfo) => void] {
  const dispatch = useDispatch()
  const activeNetwork = useSelector((state: AppState) => state.applicationInfo.activeNetworkVersion)
  const update = useCallback(
